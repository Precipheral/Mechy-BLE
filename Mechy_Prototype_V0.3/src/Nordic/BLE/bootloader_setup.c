...
{
  Serial_SendString("Jumping to bootloader");
  Delay_MS(100);

  /* Wait until ready line goes low before jump */
  while(PINB0 == 0); //(digitalRead(state->aci_pins.rdyn_pin));
    ...
    uint16_t adv_interval, uint8_t *pipes, uint8_t n_pipes)
{
  ...
  */
  ...
  crc_eeprom =  (uint16_t) eeprom_read_byte(addr++);
  crc_eeprom |= (uint16_t) (eeprom_read_byte(addr) << 8);

  /* If the two CRC values do not match, we proceed with updating the EEPROM */
  if (crc_local != crc_eeprom)
  {
    Serial_SendString("CRC does not match EEPROM. Writing new data.");

    addr = eeprom_base_addr;
    eeprom_update_byte(addr++, valid_app);
    eeprom_update_byte(addr++, valid_ble);

    for (uint8_t i = 0; i < sizeof(aci_pins_t); i++)
    {
      eeprom_update_byte(addr++, *((uint8_t *) &(state->aci_pins) + i));
    }

    eeprom_update_byte(addr++, state->data_credit_total);

    for (uint8_t i = 0; i < sizeof(pipes); i++)
    {
      eeprom_update_byte(addr++, pipes[i]);
    }

    eeprom_update_byte(addr++, timeout_l);
    eeprom_update_byte(addr++, timeout_h);
    eeprom_update_byte(addr++, interval_l);
    eeprom_update_byte(addr++, interval_h);
    eeprom_update_byte(addr++, (uint8_t) crc_local);
    eeprom_update_byte(addr++, (uint8_t) (crc_local >> 8));
     ...
     */
    addr = eeprom_base_addr;
    readback_buf = eeprom_read_byte(addr++);
